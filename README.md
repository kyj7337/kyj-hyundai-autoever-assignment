...
- MSW를 구축하여 mock API를 구성했습니다. MSW를 사용하여 클라이언트 에서 API를 구성하면, 백엔드가 작업되지 않을 상황에서도 API응답값을 받을 수 있는 환경을 구성할 수 있기 때문에, 백엔드의 작업이 끝날때 까지 기다리지 않아도 되는 장점이 있어 주로 개발할 때 사용합니다.
- 반응형 UI를 적용하며 모바일 화면에서도 적절한 화면이 보이도록 구현 했습니다. 특히 모바일 화면에서 헤더의 메뉴 버튼이 생기게 됩니다. SVG를 활용해서 위블비즈 홈페이지와 같은 애니메이션 효과를 구현하여 문제를 해결할 수 있음을 보여드릴 수 있어서 좋았습니다.
- 유닛 테스트 코드를 추가하여 데이터를 다루거나, 비즈니스 로직 유틸 함수를 테스트 할 수 있습니다. 순수함수로 구현된 함수의 테스트가 용이하기 때문에 추후 프로젝트에서도 안정성을 확보할 수 있습니다.
